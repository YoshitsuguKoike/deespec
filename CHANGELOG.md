...
* **ストリーミング出力機能**: AI対話のリアルタイム履歴記録
  - 常時有効な履歴記録（.deespec/specs/sbi/{SBI-ID}/histories/）
  - workflow_step_{N}.jsonl形式での一貫したファイル命名
  - JSONLフォーマットによるイベントストリーム記録
  - デバッグ用の生レスポンス保存（raw_response_{step}_{turn}.txt）
  - ストリーミング失敗時の自動フォールバック

* **AI応答可視化**: レビュー判定の透明性向上
  - AIレスポンス全文のコンソール表示（最初と最後の25行）
  - DECISION行の明示的な表示とログ出力
  - 短い/空の応答に対する警告表示

### 修正 (Fixed)

* **DECISION解析の改善**: 柔軟な判定抽出
  - `**DECISION: FAILED**`などのアスタリスク付き形式に対応
  - 複数の正規表現パターンによる堅牢な判定抽出
  - デフォルト値使用時の明示的なログ出力

* **無限ループ防止**: 3回失敗後の強制実装
  - attempt >= 3で判定に関わらずREVIEW&WIPステータスへ遷移
  - レビューAIによる直接実装で強制終了

* **タスク完了処理の修正**:
  - DONEステータスのSBIが再実行される問題を修正
  - ジャーナルでのDONEステータス認識（新旧両形式対応）
  - ターン番号のリセット処理追加（DONEステータス時に0へリセット）
  - WIPクリア処理の確実な実行
