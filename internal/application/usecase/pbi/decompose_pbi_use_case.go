...
func (u *DecomposePBIUseCase) Execute(
	...
	// 9. Validate and fix SBI file locations
	validationResult, err := u.validateAndFixSBILocations(pbiID)
	if err != nil {
		// Validation failed critically - update PBI status to failed
		log.Printf("SBI validation failed: %v", err)
		if updateErr := pbiEntity.UpdateStatus(pbi.StatusFailed); updateErr != nil {
			log.Printf("Failed to update PBI status to failed: %v", updateErr)
		} else {
			if saveErr := u.pbiRepo.Save(pbiEntity, pbiBody); saveErr != nil {
				log.Printf("Failed to save PBI with failed status: %v", saveErr)
			}
		}
		return &DecomposeResult{
			PBIID:          pbiID,
			...
			SBIFiles:       []string{},
			PromptFilePath: promptFilePath,
			Message:        fmt.Sprintf("SBI validation failed: %v", err),
			Prompt:         "",
		}, nil
	}

	// 10. Remove any shell scripts generated by AI
	if err := u.removeShellScripts(pbiID); err != nil {
		log.Printf("Warning: Failed to remove shell scripts: %v", err)
		// Don't fail the entire process, just log warning
	}

	// 11. List generated SBI files (after validation and fixes)
	...
	if len(sbiFiles) == 0 {
		// No SBI files after validation - mark as failed
		if updateErr := pbiEntity.UpdateStatus(pbi.StatusFailed); updateErr != nil {
			log.Printf("Failed to update PBI status to failed: %v", updateErr)
		} else {
			if saveErr := u.pbiRepo.Save(pbiEntity, pbiBody); saveErr != nil {
		...
		return &DecomposeResult{
			...
			SBIFiles:       []string{},
			PromptFilePath: promptFilePath,
			Message:        "AI executed but no valid SBI files were generated",
			Prompt:         "",
		}, nil
	}

	// 12. Create approval.yaml manifest
...
func (u *DecomposePBIUseCase) buildDecomposePrompt(
	...
	templateData := map[string]interface{}{
		"DeespecVersion":    buildinfo.GetVersion(),
		"PBIID":             p.ID,
		"Title":             p.Title,
...
// ValidationResult holds the result of SBI location validation
type ValidationResult struct {
	MovedFiles     int // Number of files moved from subdirectories
	RemovedScripts int // Number of shell scripts removed
}

// validateAndFixSBILocations validates SBI file locations and moves them if needed
// Returns ValidationResult with statistics or an error if validation fails critically
func (u *DecomposePBIUseCase) validateAndFixSBILocations(pbiID string) (*ValidationResult, error) {
	result := &ValidationResult{}

	// 1. Build PBI directory path
	pbiDir := filepath.Join(u.workingDir, ".deespec", "specs", "pbi", pbiID)

	// 2. Check for SBI files in the correct location
	correctPattern := filepath.Join(pbiDir, "sbi_*.md")
	correctFiles, err := filepath.Glob(correctPattern)
	if err != nil {
		return nil, fmt.Errorf("failed to list SBI files in correct location: %w", err)
	}

	// 3. Check for SBI files in subdirectories
	subdirPattern := filepath.Join(pbiDir, "*", "sbi_*.md")
	subdirFiles, err := filepath.Glob(subdirPattern)
	if err != nil {
		return nil, fmt.Errorf("failed to list SBI files in subdirectories: %w", err)
	}

	// 4. Move files from subdirectories to correct location
	for _, subdirFile := range subdirFiles {
		fileName := filepath.Base(subdirFile)
		targetPath := filepath.Join(pbiDir, fileName)

		// Check if target already exists
		if _, err := os.Stat(targetPath); err == nil {
			log.Printf("Warning: Target file %s already exists, skipping move", fileName)
			continue
		}

		// Move the file
		if err := os.Rename(subdirFile, targetPath); err != nil {
			log.Printf("Warning: Failed to move %s to correct location: %v", fileName, err)
			continue
		}

		log.Printf("Moved SBI file from subdirectory: %s -> %s", subdirFile, targetPath)
		result.MovedFiles++
	}

	// 5. Check if any SBI files exist after moving
	allFiles, err := filepath.Glob(correctPattern)
	if err != nil {
		return nil, fmt.Errorf("failed to list SBI files after moving: %w", err)
	}

	// 6. If no SBI files in correct location and none were moved, check outside PBI directory
	if len(allFiles) == 0 && len(correctFiles) == 0 && len(subdirFiles) == 0 {
		// Check parent directory for misplaced SBI files
		parentDir := filepath.Dir(pbiDir)
		parentPattern := filepath.Join(parentDir, "sbi_*.md")
		parentFiles, err := filepath.Glob(parentPattern)
		if err != nil {
			return nil, fmt.Errorf("failed to check parent directory: %w", err)
		}

		if len(parentFiles) > 0 {
			// Files are in wrong location (outside PBI directory) - this is a critical error
			return nil, fmt.Errorf("SBI files found outside PBI directory: %v", parentFiles)
		}
	}
...
// removeShellScripts removes any shell script files generated by AI
// Looks for common shell script patterns and removes them
func (u *DecomposePBIUseCase) removeShellScripts(pbiID string) error {
	// 1. Build PBI directory path
	pbiDir := filepath.Join(u.workingDir, ".deespec", "specs", "pbi", pbiID)

	// 2. Define shell script patterns to remove
	...
	// 3. Check each pattern and remove matching files
	for _, pattern := range scriptPatterns {
		fullPattern := filepath.Join(pbiDir, pattern)
		...
		for _, match := range matches {
			...
			if err != nil {
				log.Printf("Warning: Failed to stat file %s: %v", match, err)
				continue
			}

			if !info.IsDir() {
				if err := os.Remove(match); err != nil {
					log.Printf("Warning: Failed to remove shell script %s: %v", match, err)
					continue
				}
				log.Printf("Removed shell script: %s", match)
				removedCount++
			}
		}
	}
