...
func Clear(paths app.Paths, opts ClearOptions) error {
	...
	Info("Clear completed successfully. Archive created at: %s\n", archiveDir)
	return nil
}
...
func archiveJournal(journalPath, archiveDir string) error {
	...
	if _, err := os.Stat(journalPath); os.IsNotExist(err) {
		Info("No journal.ndjson to archive\n")
		return nil
	}
...
func archiveSpecs(archiveDir string) error {
	...
	if _, err := os.Stat(specsDir); os.IsNotExist(err) {
		Info("No specs directory to archive\n")
		return nil
	}
	...
	Info("Moved %d items from specs to archive\n", movedCount)
	return nil
}
...
func resetStateFiles(paths app.Paths) error {
	...
	if err := saveStateCAS(paths.State, initialState, 0); err != nil {
	...
	if err := os.WriteFile(paths.Health, healthData, 0644); err != nil {
		...
		Warn("Failed to reset health.json: %v\n", err)
	}
	...
	for _, entry := range entries {
		if entry.IsDir() {
			...
			if err := os.RemoveAll(archivePath); err != nil {
				Warn("Failed to delete archive %s: %v\n", entry.Name(), err)
			} else {
				deletedCount++
			}
		}
