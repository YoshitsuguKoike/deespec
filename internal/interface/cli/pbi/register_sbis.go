...
// registerSBIsFlags holds flags for the register-sbis command
type registerSBIsFlags struct {
	dryRun bool
	force  bool
}

// NewRegisterSBIsCommand creates a new register-sbis command
func NewRegisterSBIsCommand() *cobra.Command {
	flags := &registerSBIsFlags{}

	cmd := &cobra.Command{
		Use:   "register-sbis <pbi-id>",
...
can be executed using 'deespec run' or 'deespec sbi list'.`,
  ...
  deespec pbi register-sbis PBI-001

  # Preview registration without saving (dry-run mode)
  deespec pbi register-sbis PBI-001 --dry-run

  # Force re-registration of already registered SBIs
  deespec pbi register-sbis PBI-001 --force`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			pbiID := args[0]
			return runRegisterSBIs(pbiID, flags)
		},
	}
...
func runRegisterSBIs(pbiID string, flags *registerSBIsFlags) error {
	...
	if flags.dryRun {
		...
		fmt.Printf("   $ deespec pbi register-sbis %s\n", pbiID)
		return nil
	}
