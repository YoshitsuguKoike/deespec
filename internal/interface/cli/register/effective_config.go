...
func runPrintEffectiveConfig(cliCollisionMode, format string, compact, redactSecrets bool) error {
	...
	originalLevel := common.GetLogger().GetLevel()
	common.GetLogger().SetLevel(common.LogLevelInfo)
	...
	if err != nil && !os.IsNotExist(err) {
		...
		common.Error("failed to load policy: %v\n", err)
		return err
	}

	// Log policy load status (use Info for diagnostic command)
	if policyFileFound {
		common.Info("policy loaded: %s", policyPath)
	} else {
		common.Info("no policy file found, using defaults")
	}
	...
	case "yaml":
		...
		if err != nil {
			common.Error("failed to marshal to YAML: %v\n", err)
			return err
		}
	case "json":
		if compact {
		...
		if err != nil {
			common.Error("failed to marshal to JSON: %v\n", err)
			return err
		}
	...
	default:
		common.Error("unsupported format: %s\n", format)
