...
### End ###`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := ExtractNoteBody(tt.content, tt.noteType)
			if result != tt.expected {
				t.Errorf("ExtractNoteBody() for %s = %q, want %q",
...
func TestAppendNoteBasic(t *testing.T) {
	...
	err := AppendNote("implement", "PENDING", "Test note body", 1, "TEST-001", testTime)
	if err != nil {
